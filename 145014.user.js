...
////////////////////
	function checksearch(){
		var list = document.getElementById('ires');//結果區域
		if(list){
    		document.removeEventListener('DOMNodeInserted',checksearch,false);//wtf is checksearch
	...
	function clear(){
		...
		for(var i =0;i<items.length;i++){
    		items[i].removeAttribute('onmousedown');//cancel 所有給google的返回值
    	}
	}
...
;(function() {//a self invoking function
    ...
    document.addEventListener('click', function(e){
        var a = e.target;
        a.href && (a.href = a.href.replace(re, '$1'));//get the first part of the reg expression matching
    });
})();

String.prototype.toObj = function(s) {//make a dictionary
